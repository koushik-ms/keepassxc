...
Now we can see two files wre generated:
...
Please read the manual on how to use PuTTYgen for details on generate a key: https://www.ssh.com/ssh/putty/putty-manuals/0.68/Chapter8.html#pubkey-puttygen. Once generated, you must save the key in OpenSSH format, follow the image below.

.Generating Key with PuttyGen
...
2. Set the password you used for the keyfile in the password field.
3. Go to the advanced category and attach the keyfile you generated previously.
