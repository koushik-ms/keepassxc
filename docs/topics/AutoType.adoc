...
|{DELAY=X}     |Set keypress delay to X milliseconds
...
Pressing the global Auto-Type hotkey cause KeePassXC to searche the database for entries that match the window title. Multiple matches may be returned and will cause the sequence selection dialog to appear. Click on a sequence line will immediately execute the Auto-Type action. A search box is also available in case numerous matches are returned.
...
WARNING: Be careful when using Entry-Level Auto-Type as you can inadvertantly type into the wrong window. For example, a chat window or email.
